...
## 项目描述
每日记是一个面向个人记录场景的 Web 应用，重点放在“长期保存、快速记录、随时回看”这三个核心体验上。它不是一个社交型内容平台，而是一个更偏向私密、轻量、可控的数据工具：你可以按日期写日记、随手记录独立的碎碎念、回顾历史内容、搜索过往记录，并把数据保留在自己的 SQLite 数据库里。

项目采用前后端分离的单页应用结构。前端负责日记和碎碎念编辑、历史浏览、搜索和设置等交互，后端负责认证、数据持久化、导入导出以及可选的 LLM 自动标题生成。整体设计尽量保持简单：界面直观、接口清晰、部署方式也尽量少依赖外部服务，适合本地运行、Docker 部署或放到个人服务器上使用。
...
- **独立碎碎念**: 只记录正文，不需要标题；数据保存在独立的 SQLite 表中，不会混入日记，也不计入日记统计
- **轻量写作**: 不做 Markdown、图片、视频和复杂富文本，只保留最直接的记录体验
- **情绪与状态**: 记录 Mood、Fulfillment、基础天气和氛围天气，帮助回看当天的状态
  - Mood和Fulfillment滑块记录当天的心情状态，Mood越高表示当天越快乐，Fulfillment越高表示当天越忙越充实
  - 基础天气滑块记录当天主要天气情况
  - 氛围天气记录当天次要天气情况，此组件不是滑块，是多选列表
- **日历式回顾**: 通过日历和历史列表快速回到任意一天
- **连续浏览**: 历史、搜索结果和碎碎念在接近页面底部时自动加载下一页
- **正文展开**: 可以直接在历史和搜索结果中展开或收起较长的日记正文
- **全文搜索**: 可以按内容、日期等条件快速找到旧记录
- **统计页面**: 查看一段时间内的记录概览和趋势
- **LLM 总结**: 可选地自动生成标题，让长日记更容易扫读。如不启用默认使用日记正文前几个字符作为标题
- **数据导入导出**: 支持通过同一份 JSON 文件备份和迁移日记与碎碎念
- **基础个性化**: 设置用户名、头像和主题外观
